if ($Online) {
  ...
  ForEach-Object { ([regex]'http.+-windows-x64.exe').Matches($_) | ForEach-Object { $_.Value } } | Select-Object -First 1
  ...
  if (!(Test-Path $JdkInstalledFolder)) {
    ...
    Sort Length -Descending | Select-Object -First 1 | ForEach-Object { $_.FullName }
    ...
    ForEach-Object { & $Unpack200 -r "$_" "$($_.Directory)\$([io.path]::GetFileNameWithoutExtension($_)).jar" }
